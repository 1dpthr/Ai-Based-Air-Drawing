...
def recognize_shape(points):
    ...
    closed = gap < path_len * 0.45

    candidates = []

    # ── Open stroke → line ──
    if not closed or gap / path_len > 0.60:
...
def apply_stroke_correction(canvas, backup_canvas, stroke, color, thickness, enabled=True):
    ...
    Always recognizes shape for feedback. Returns (new_canvas, shape_type or None, confidence).
    """
    if backup_canvas is None or len(stroke) < MIN_POINTS:
    ...
    if not enabled:
        return canvas, shape_type, confidence
